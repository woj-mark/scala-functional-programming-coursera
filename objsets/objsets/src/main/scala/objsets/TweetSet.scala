...
abstract class TweetSet extends TweetSetInterface:
   ...
   */
  //Doing a trick hre= use filterAcc with an empty set
...
class Empty extends TweetSet:
  //Return acc for an emptry set
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = acc

  //Implemented in the subclass of the TweetSet
  def union(that: TweetSet): TweetSet = {
    that 
  }
  /**
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("This is an empty TweetSet, no re-tweets.")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = 
    {//Mutable search with a foreach through tweets
    var most:Tweet = elem //Initialize the viable for search
    this.foreach(tw => 
      if(tw.retweets>most.retweets) most = tw
      ) 
    most
    }
  ...
  override def descendingByRetweet: TweetList = {
    ...
    new Cons(tm, remove(tm).descendingByRetweet)
  }
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = {
    TweetReader.allTweets.filter(tw => google.exists(word => tw.text.contains(word)))
  }

  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter(tw => apple.exists(word => tw.text.contains(word)))
  }
   ...
   * 
   * Us union on the apple and google tweetsets and apply the descendingByRetweet method
   */
